...
const Reward: FC<PropTypes> = function(props) {
  ...
  function createDefaultAssets() {
    const [mask, tiger, stars, countGroup, flyStar] = [createMask() , createTiger(), createStars(), createCountGroup() , createFlyStar()]
  ...
  function createStars() {
    ...
    for (let index = 1; index <= props.star; index++) {
      const w = 108, h = 105, centerW = vpWidth / 2, centerH = vpHeight / 2 + 20
      const xMap = {
        1: centerW,
        2: centerW + (index === 1 ? -(55 + w / 2 ) : 55 + w / 2),
        3: index === 2 ? centerW : index === 1 ? centerW + (-(w + 70)) : centerW + w + 70,
      },
      yMap = {
        1: centerH,
        2: centerH,
