...
const disableDraggingTiger = false;
const disableAutoJumping = false;
...
const Delivery: FC<PropTypes> = function (props) {
    ...
    function bindEvents() {
        ...
        const [tiger, backward] = findNamesByLayer(stage.layer, ['tiger', 'backward']);
        ...
        if (tiger) {
            ...
            configs.circle.coordinate.forEach((rows, x) => {
                rows.forEach((pos, y) => {
                    findNamesByLayer(stage.layer, [`circle-${x}-${y}`])[0]
                        .addEventListener(EvtNameEnum.TOUCH_START, () => {
    ...
    function isNeighbour([x, y]) {
        ...
        return (anchorX === x && Math.abs(anchorY - y) === 1)
    ...
    function onTouchMoving(e) {
        ...
        const restore = fnRestore.bind({}, {
            obj: tiger, pos: { x: anchorX, y: anchorY }
        }, { obj: walking });
        const moveHorizontal = fnMoveHorizontal.bind({}, {
            obj: tiger, pos: { x: anchorX, y: anchorY }
        }, { obj: walking, pos: { x: polyX, y: polyY } });
        const moveVertical = fnMoveVertical.bind({}, {
            obj: tiger, pos: { x: anchorX, y: anchorY }
        }, { obj: walking, pos: { x: polyX, y: polyY } });
     ...
     * 将小老虎恢复原来的位置
     */
    function fnRestore(tiger, walking) {
        !disableDraggingTiger && tiger.obj.attr({ pos: [tiger.pos.x, tiger.pos.y] });
        !disableDraggingLines && walking.obj.attr({ points: [] });
    }

    /**
     * 让小老虎随手指拖动，水平移动
     */
    function fnMoveHorizontal(tiger, walking, pos) {
        !disableDraggingTiger && tiger.obj.attr({ pos: [pos[0], tiger.pos.y] });
        !disableDraggingLines && walking.obj.attr({ points: [walking.pos.x, walking.pos.y, pos[0], walking.pos.y] });
    }

    /**
     * 让小老虎随手指拖动，垂直移动
     */
    function fnMoveVertical(tiger, walking, pos) {
        !disableDraggingTiger && tiger.obj.attr({ pos: [tiger.pos.x, pos[1]] });
        !disableDraggingLines && walking.obj.attr({ points: [walking.pos.x, walking.pos.y, walking.pos.x, pos[1]] });
    }
    ...
    function update(append = null, remove = null) {
        const [tiger, polyline] = findNamesByLayer(stage.layer, ['tiger', 'polyline']);
        ...
        // 增量绘制新增的圆圈
        if (append) {
            const [circle] = findNamesByLayer(stage.layer, [`circle-${append[0]}-${append[1]}`]);
            circle && circle.attr({
                bgcolor: configs.circle.color,
                border: [configs.circle.border.width, configs.circle.border.color],
            });
        ...
        // 增量绘制回退的圆圈
        if (remove) {
            const [circle] = findNamesByLayer(stage.layer, [`circle-${remove[0]}-${remove[1]}`]);
